...
export default function Remove() {
        ...
        <div className="grid gap-1 pb-6">
          <div className="flex items-center justify-between">
            <div className="text-sm text-secondary">
              {i18n._(t`${currencyA?.symbol}/${currencyB?.symbol} will be removed`)}
            </div>
  ...
  return (
    <Container id="remove-liquidity-page" className="py-4 space-y-4 md:py-8 lg:py-12" maxWidth="2xl">
      <Head>
        <title>Remove Liquidity | HolyGrail</title>
        <meta key="description" name="description" content="Remove liquidity from the HolyGrail DFK AMM" />
      </Head>
                        ...
                        {chainId && (oneCurrencyIsWETH || oneCurrencyIsETH) ? (
                          <RowBetween className="text-sm">
                            {oneCurrencyIsETH ? (
                              ...
                              <Link
                                href={`/remove/${currencyA?.equals(WNATIVE[chainId]) ? 'ONE' : currencyIdA}/${
                                  currencyB?.equals(WNATIVE[chainId]) ? 'ONE' : currencyIdB
                                }`}
                              >
                                <a className="text-baseline text-yellow opacity-80 hover:opacity-100 whitespace-nowrap">
